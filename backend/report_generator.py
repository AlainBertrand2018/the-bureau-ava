...
class BureauReportGenerator:
    ...
    def generate_dossier(data: Dict[str, Any], report_type: str = "GENESIS") -> str:
        ...
        for i, q in enumerate(instrument):
            ...
            clean_q = BureauReportGenerator._clean_question(q)
        ...
        base_disclosure = manual.get("scientific_disclosure", "Built on Psychometric Measurement Theory and Agent-Based Modeling.")
        disclosure = f"""
        <p>{base_disclosure}</p>
...
<body>
    ...
    def _clean_question(q_text: str) -> str:
        """Removes the trailing parenthetical options from the question text."""
        import re
        # Remove trailing parentheses containing options or separators
        cleaned = re.sub(r'\s*\([^)]*?(?:=|times|satisfied|agree|disagree|never|often|yes|no|always|maybe)[^)]*\)$', '', q_text.strip(), flags=re.IGNORECASE)
        return cleaned

    @staticmethod
    def _parse_field_options(q_text: str) -> List[str]:
        """
        Extracts options from survey questions (usually in parentheses at the end).
        Example: "How are you? (1=Bad, 5=Good)" -> ["1=Bad", "5=Good"]
        Example: "Gender? (Male, Female, Other)" -> ["Male", "Female", "Other"]
        """
        import re
        ...
        if match:
            ...
            for sep in [',', '/', ';']:
                if sep in options_raw:
                    return [o.strip() for o in options_raw.split(sep)]
            
            # If no common delimiter, check if it's a "dash" separated list like "Option A - Option B"
            if " - " in options_raw:
                return [o.strip() for o in options_raw.split(" - ")]
                
            # Fallback: if it's just one word, return it as a single option
            return [options_raw.strip()]
